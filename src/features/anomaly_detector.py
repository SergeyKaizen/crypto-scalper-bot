...
class AnomalyDetector:
    def __init__(self):
        self.quiet_streaks = {}  # symbol -> int (FIX race condition)

    def detect_anomalies(self, features_by_tf: dict, current_tf: str, symbol: str = "UNKNOWN") -> dict:
        """
        ...
        for tf in TF_ORDER:
            if tf in features_by_tf:
                tf_anomalies[tf] = self._detect_single_tf(features_by_tf[tf], tf)
        ...
        for window, single_anom in current_anomalies.items():
            ...
            # Quiet streak per-symbol
            if symbol not in self.quiet_streaks:
                self.quiet_streaks[symbol] = 0
            if anomaly_type == 'Q':
                self.quiet_streaks[symbol] += 1
            else:
                self.quiet_streaks[symbol] = 0

            # Consensus
            consensus_count = self._get_consensus_score(anomaly_type, tf_anomalies, current_tf)
            ...
            result[window] = {
                ...
                'quiet_streak': self.quiet_streaks[symbol],
    ...
    def _detect_single_tf(self, features: dict, timeframe: str) -> dict:
        """Детекция аномалий на одном TF по всем окнам"""
        anomalies = {}
        for window, w_feats in features.items():
            half_feats = w_feats.get('half_changes', {})

            candle_ok = self._check_candle_anomaly(w_feats, half_feats)
            volume_ok = self._check_volume_anomaly(w_feats, half_feats)
