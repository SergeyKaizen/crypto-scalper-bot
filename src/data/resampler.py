...
"""
Инкрементальный ресэмплер 1-минутных свечей в старшие таймфреймы.
...
class Resampler:
    ...
    def add_1m_candle(self, candle: dict) -> None:
        """
        Добавляем новую 1-минутную свечу (приходит из websocket_manager или polling).
        ...
        # Ресэмплим и добавляем в старшие TF
    ...
    def get_window(self, tf: str, window_size: int) -> Optional[pl.DataFrame]:
        ...
        Если в кэше недостаточно — возвращает None (нужно докачать из storage).
        """
    ...
    def get_all_windows(self, tf: str) -> Dict[int, pl.DataFrame]:
        """
        Удобный метод для модели: возвращает все нужные окна одним вызовом.
